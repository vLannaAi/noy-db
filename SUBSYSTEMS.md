...
|---|---|---|
| `@noy-db/hub/partitioning` | Time-range / region / tenant partition awareness for query execution. The 37 LOC of dormant plumbing in `query/join.ts` (`partitionScope: 'all'` + reducer `seed`) is the seed for this. | v0.11+ |
...
- Should `lazy` mode (cache + on-demand fetch) be promoted from inside `routing` to its own headline subsystem? Trade-off: clarity vs. catalog inflation. Defer to v0.26 review.
