...
| **v0.23.0** | 🟢 phases A+B+C shipped | **Lazy-mode indexes — queries, range, composite, auto-reconcile.** Phase A: write-path ([#266](https://github.com/vLannaAi/noy-db/issues/266)), equality + orderBy dispatch ([#267](https://github.com/vLannaAi/noy-db/issues/267), [#268](https://github.com/vLannaAi/noy-db/issues/268)), rebuild/reconcile ([#269](https://github.com/vLannaAi/noy-db/issues/269)), node-side bench baseline ([#270](https://github.com/vLannaAi/noy-db/issues/270) partial). Phase B: range predicates ([#275](https://github.com/vLannaAi/noy-db/issues/275)) with typed-value comparison — no more `'10' < '2'`. Composite indexes over `[f1, f2, ...]` tuples ([#276](https://github.com/vLannaAi/noy-db/issues/276)) — planner prefers composite when `==` clauses cover every component. Phase C: `reconcileOnOpen: 'auto' \| 'dry-run'` ([#278](https://github.com/vLannaAi/noy-db/issues/278)) — first lazy-mode query auto-reconciles each declared field and emits `index:reconciled`. FTS ([#277](https://github.com/vLannaAi/noy-db/issues/277)) and browser-side 250ms bench gate remain open. |
