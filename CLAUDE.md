...
**Status:** v0.10.0 on npm (2026-04-09). All 14 `@noy-db/*` packages on the 0.10.0 version line (core, store-file, store-memory, store-browser, store-dynamo, store-s3, vue, nuxt, pinia, yjs, auth-webauthn, auth-oidc, create-noy-db, plus any others). Full v0.5–v0.9 surface **plus** the v0.10 store rename. 1065 tests passing.
...
packages/
  ...
  store-dynamo/  # @noy-db/store-dynamo — DynamoDB single-table store
  store-s3/      # @noy-db/store-s3 — S3 store
  store-memory/  # @noy-db/store-memory — in-memory store (testing)
  store-browser/ # @noy-db/store-browser — localStorage/IndexedDB store
...
All stores implement exactly 6 async methods:
