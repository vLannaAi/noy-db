...
The sections above are the *user* story. This section is the *developer* story: runnable snippets for every feature. Each example assumes the Nuxt 4 + `@noy-db/nuxt` happy path from [`getting-started.md`](./getting-started.md).
...
Peak memory stays under 200 MB on a 100K-record collection.

## Lazy hydration + LRU eviction

By default a compartment eager-loads every record on open. Set `cache` and lazy mode kicks in: `get(id)` hits the adapter on cache miss and populates the LRU; `list()` and `query()` throw (use `scan()` or `loadMore()`); `indexes` are rejected at construction because they require full hydration.
...
Every record can be schema-validated, every mutation is recorded in a tamper-evident ledger, and every backup is verifiable end-to-end.
...
Known limitation: reconstruct is ambiguous across delete+recreate cycles because the version counter resets.
...
Cycle-safe cascade (mutually-cascading collections terminate). Cross-compartment refs are rejected with `RefScopeError`.
