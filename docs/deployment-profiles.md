...
# Pinia / Nuxt layer: also @noy-db/pinia and @noy-db/nuxt
```

```mermaid
flowchart TB
    subgraph Nuxt["Nuxt Application"]
        Composables["useNoydb / useCollection /<br/>useQuery / useSync<br/>(or @noy-db/pinia stores)"]
...
**Pinia integration:** see profiles 7a and 7b below for the two idiomatic Nuxt 4 topologies.

---

## 7a. Nuxt 4 + browser adapter + Dynamo sync

The default web topology: store everything in the browser (localStorage / IndexedDB), sync to DynamoDB when online. Auto-imports + Pinia stores via `@noy-db/nuxt`. SSR-safe (client-only runtime plugin).
...
Adapters compose. For example, `withCache()` turns any remote adapter into a cache-first adapter:
