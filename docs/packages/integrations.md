...
| Package | When to use |
|---|---|
| [`@noy-db/in-vue`](../../packages/in-vue) | Vue 3 composables — `useNoydb`, `useCollection`, `useSync`. Works outside Nuxt too. |
| [`@noy-db/in-pinia`](../../packages/in-pinia) | Pinia store factory — `defineNoydbStore<T>()`. Typed, reactive, SSR-safe. |
