...
back into records, build a preview diff via `diffVault()`, and
...
| `@noy-db/as-blob` | `fromBytes(vault, bytes, { collection, id, slot?, policy?, mimeType?, filename? })` | `AsBlobImportPlan` |
| `@noy-db/as-xml` | `fromString(vault, xml, { collection, recordElement?, fieldTypes?, policy? })` | `AsXMLImportPlan` |
| `@noy-db/as-xlsx` | `fromBytes(vault, bytes, { collection, sheet?, headerRow?, fieldTypes?, dicts?, policy? })` | `AsXlsxImportPlan` |
...
### Dict-label inversion (`as-xlsx`)

`fromBytes` inverts human-readable labels back to their stable dict keys before building the `ImportPlan`. Supply a `dicts` option (explicit, takes precedence) or populate vault dictionaries with `withI18n()` for the automatic path:

```ts
import { withI18n } from '@noy-db/hub/i18n'
const db = await createNoydb({ store, user, secret, i18nStrategy: withI18n() })
...
// xlsx written with 'Paid'/'Draft' in the status column:
const plan = await fromBytes(vault, xlsx, { collection: 'invoices' })
// plan rows carry 'paid'/'draft' (stable keys), not the human labels
```

Or supply dicts explicitly for fresh-vault imports where the dictionary isn't yet populated:

```ts
const plan = await fromBytes(vault, xlsx, {
  collection: 'invoices',
  dicts: {
    status: [
...
Unknown labels (no match in any locale) pass through as-is. If two different keys share the same label across any locale, `XlsxDictAmbiguityError` is thrown with the offending column and label.

### Deferred follow-ups

- Per-import ledger tagging (`reason: 'import:<format>'`) — audit consumers can distinguish manual edits from imports. Scope larger than initial estimate; see issue thread.
