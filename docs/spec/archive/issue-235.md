...
- `docs/START_HERE.md` — add a "Hub subpaths" subsection under "The mental model" showing which subpath to import from for which concern. Keep the main-entry example as the default (backward compat).
- `docs/topology-matrix.md` — add a "Tree-shaking recipe" note to each pattern showing subpath imports for the opted-in case.
