...
- **`forget()` MUST fan out through the full purge suite — hardened by audit.** The single non-negotiable invariant: forgetting `msgs/x` while `msgs_text/x` retains the body is a plaintext leak of exactly the secret `forget` promised to shred (same discipline as the classified/dekResidue work). Three mechanism rules against the actual forget path (`vault.forget(subjectId)`, subject-index-driven, opt-in via `withForgetCascade`):
  ...
  3. **A satellite of a forget-covered base MUST be `perRecordKeys` (R-S7), and retro-coverage requires migration.** A genuine crypto-shred requires the per-record CEK; without R-S7 the heaviest, most sensitive fields would get the weak shred while the hot fields get the strong one — inverted protection. Enforced as a loud declaration-time refusal (never silent auto-enable of a key mode). **Audit addition:** `perRecordKeys` is construction-only (`collection.ts:438`), so adding forget coverage to a base whose satellite *already exists* without it is refused until an explicit satellite CEK migration has run — a refusal alone would leave an un-erasable residue population behind an apparently-covered base. **The way forward (#599, 2026-07-14):** `Vault.migrateSatellitePerRecordKeys(satelliteName)` — see § Implementation amendments — walks the satellite via `_applyCutoverTransform`, minting a distinct per-record CEK for every record; run it before the redeclare that adds `perRecordKeys: true`, and R-S7 then passes.
...
- **Rotation** — CEK/DEK rotation is per-collection; base and satellite are independent collections with their own DEKs, so no special coupling beyond the delete/forget fan-out. The `perRecordKeys` question is settled by R-S7 + the retro-migration clause (`Vault.migrateSatellitePerRecordKeys`, #599).
...
- **R-S7 retro-coverage gets a migration path: `Vault.migrateSatellitePerRecordKeys` (#599, 2026-07-14).**
  R-S7 correctly refuses declaring/adding forget coverage over a satellite that lacks `perRecordKeys`,
  but v1 originally shipped no way *past* the refusal — `perRecordKeys` is construction-only
  (`collection.ts:438`) and the one per-record re-encrypt primitive, `_applyCutoverTransform`, was wired
  only to the generic vault-wide `schema-update` cutover fence, itself requiring the collection already
  constructed in the target mode (chicken-and-egg). The fix does **not** touch `declare.ts` or the R-S7
  gate itself: `Vault.migrateSatellitePerRecordKeys(satelliteName)` opens the satellite via the ordinary,
  already-public `vault.collection(name, { perRecordKeys: true })` path — omitting `satelliteOf` so
  `declareSatellite` (and R-S7) is never invoked for the migration pass — then calls
  `coll._applyCutoverTransform((doc) => doc)` (an identity transform reusing the existing per-record CEK
  mint-or-reuse logic verbatim, `with-shape/satellites/migrate-cek.ts`). Every record is re-encrypted
  under a distinct, freshly-minted per-record CEK; a **separate**, later `vault.collection(name, {
  satelliteOf, perRecordKeys: true })` call still runs the unmodified R-S7 check and now passes.
  **Resumable for free:** `_applyCutoverTransform`'s CEK resolution (`resolveStableCek`) reuses a
  record's existing `_cek` if one is already present and mints only when absent, so re-running the
  migration after a mid-walk failure is safe — already-migrated records keep their exact CEK (re-wrapped,
  fresh IV) rather than being re-minted; a resumed run does re-touch already-migrated records (fresh IV,
  bumped `_v`, a duplicate ledger `migration` entry) rather than skipping them, which is accepted
  write-amplification, not a correctness gap. **Precondition (undocumented hazard, closed by a
  mode-assertion guard):** because `Vault`'s collection cache is first-wins with no mismatch guard, a
  satellite name already opened *this session* without `perRecordKeys` (e.g. by unrelated code) would
  cache-hit and silently re-encrypt under the shared collection DEK instead of minting per-record CEKs —
  `migrateSatellitePerRecordKeys` now asserts `perRecordKeys` mode via `getConfig()` before walking and
  throws `SatelliteConfigError` otherwise. Run the migration before the satellite collection serves any
  other traffic this session (no vault-wide fence/quiesce is held — deliberately, to avoid pulling in the
  disproportionately heavy `schema-update` fence machinery for a scoped single-collection re-key; a
  concurrent write to the same satellite during the walk is a documented, not fenced, hazard).
