...
- **Sub-slice 3b — the cooperative ack-barrier (multi-client quiesce).** 3a already quiesces *passively* (every write reads the live fence, so the moment it goes `draining` all new writes block with `SchemaFenceError`). 3b adds the **active barrier** that closes the in-flight-write window:
  - **Client registry (store-based, purpose-built — NOT `Collection.presence`):** each client writes `_meta/schema-fence:client:<clientId>` = `{ lastSeen, quiescedAtVersion | null }` on a heartbeat interval. `clientId` is minted per `Noydb` instance. (Presence is user-scoped/per-collection; a dedicated fence registry is simpler.)
  - **Fence watcher (per client):** polls the fence (reuses `loadFence`); on `draining` → `await hub.writeQueue.onFlush()` (drain in-flight) → stamp `quiescedAtVersion` → emit the local `schema:fence-changed` signal. On `complete` with a bumped counter → surface "reload required." Tests drive it via an explicit `check()` (no fake timers).
  - **Migrator `runCutover` (extends 3a):** after `draining`, compute the **active set** (registry docs with `lastSeen` within `staleMs`) and wait until every active client has `quiescedAtVersion === draining-generation`, or the 60s timeout (stale clients drop out). Then `migrating` → transform → bump → `complete`. 3a's single-client path is the degenerate case.
  - **Resume/abort:** opening a vault to `fenceState ∈ {draining, migrating}` (a cutover that died mid-flight) → `runSchemaCutover()` resumes; `abortSchemaCutover()` resets to `normal` at the current generation.
  - **`schema:fence-changed`** on `NoydbEventEmitter` — same-instance only (for #233's UI); cross-client coordination goes through the store, not this event.

  **Explicitly NOT in 3b (deferred):** `by-peer` Web Locks **leader election / migrator failover** — the client that calls `runSchemaCutover()` is the migrator. Reactive cross-client fence *push* (pub/sub) is a later optimization; the baseline is store-polling.

  **Test harness (settled):** N `Noydb` instances on one shared `memory()` store, in one process; heartbeats + watcher `check()` driven explicitly; assert the migrator blocks until all active clients ack, proceeds on timeout when one goes stale, and that an un-quiesced in-flight write is what the barrier waits out. No `BroadcastChannel` (absent in the Node test env) and no `MinimalLockManager` needed (election deferred).
...
- 3b test harness — SETTLED (§5 Slice 3b): N `Noydb` instances on one shared `memory()` store, heartbeats + watcher `check()` driven explicitly; no `BroadcastChannel` / `MinimalLockManager` (election deferred).
