...
- **Late-join state sync (real-time providers) — KNOWN LIMITATION, follow-up:** a writer that opens a vault *during* an in-progress cutover over a `by-*` channel does not learn the active fence (channels don't replay state) — its `readFence` returns `normal` and `assertWritable` would not block it. The store default is unaffected (it polls the live fence doc). v1 assumes a stable writer set across a (brief, admin-triggered) cutover — the normal schema-migration case, and the barrier's quorum is over *present* writers anyway. Closing it is a join-handshake (on connect, broadcast a sync-request; reachable ends re-broadcast their current fence + presence) — a clean follow-up, out of v1 scope.
