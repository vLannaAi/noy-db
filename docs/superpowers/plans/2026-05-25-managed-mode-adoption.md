...
**RESOLVED (advisor, 2026-05-28): (B) as an ORCHESTRATOR.** Don't replicate the bypass-open-enroll-reassert window — **call the existing public `Noydb.openVaultAndEnrollRecovery`** as a normal client. After Task 1 the adopted store IS a managed-mode vault on disk (sealed passphrase + owner keyring with wrapped partition DEKs), so Task 2 just: construct `createNoydb({ store, user, passphraseMode: 'managed', sealingKey, shamirRecovery })` and call `db.openVaultAndEnrollRecovery(vaultName, { recovery })`. The gate dance lives where it already lives; the bundle code is an orchestrator, not a reimplementer. **Verified** (`noydb.ts:2018`): `openVaultAndEnrollRecovery` calls `this.openVault(vault)` — works on an existing managed vault (managed open resolves the already-sealed passphrase via `resolveManagedSecret`'s reopen arm; loads the existing keyring, doesn't recreate). **Use a dynamic `import('../noydb.js')`** in the managed branch only — keeps the `Noydb` class out of the `@noy-db/hub/bundle` static graph (bundle-size gate). Advisor pins: (1) discriminator is `passphraseMode` (same as createNoydb, no new vocab); (2) validate managed ⇒ `recovery` has a Shamir profile + `shamirRecovery` provider present, BEFORE any disk write, error naming `@noy-db/on-shamir`; (3) document idempotent retry — a half-done managed adoption (keyring+seal written, recovery failed) re-runs cleanly because `resolveManagedSecret` reopen-arm reuses the existing sealed passphrase; (4) test with the real `@noy-db/on-shamir` provider, not a mock.
