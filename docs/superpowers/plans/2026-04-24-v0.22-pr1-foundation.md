...
| `packages/hub/src/errors.ts` | modify | Add `IndexRequiredError` (code `'INDEX_REQUIRED'`) and `IndexWriteFailure` (code `'INDEX_WRITE_FAILURE'`) |
...
- [ ] **Step 1.1 — Add `IndexRequiredError` and `IndexWriteFailure` class source.**
...
```
Expected: compile error / export missing — `IndexRequiredError` / `IndexWriteFailure` not exported from `../src/errors.js`.
...
git commit -m "feat(hub): add IndexRequiredError + IndexWriteFailure typed errors
...
- Adds typed `IndexRequiredError` + `IndexWriteFailure` errors.
...
- `IndexWriteFailure` payload `{ recordId, field, op, cause }` — consistent across Task 1 tests, class source, and spec §3.4.
