...
- [ ] **Step 4: green** + regression (`persisted-schemas/*`, `schema-field-ids.test.ts`, `bundle-roundtrip.test.ts`) + typecheck + check:architecture.
- [ ] **Step 5: commit** — `feat(hub): derive schema manifest from per-collection schemas + round-trip-stable sync (#941)`
