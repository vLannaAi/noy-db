...
**Smart substitute (#285).** `smartSubstitute: true` makes the `'substitute'`
fallback *script-aware*: after the explicit `substitute` chain misses, it picks
the available locale whose primary script is **nearest** the target — same
script first, then **Latin** (broadly readable), then any other — instead of an
arbitrary first-non-empty value. So a missing Thai label falls back to another
Thai (or a Latin) translation rather than, say, an Arabic one. Default `false`.
...
i18nText({
  ...
  smartSubstitute: true,   // #285 — nearest-script fallback
})
```

> **Current wiring (ALL layers):** `read` (`get`/`list`), `guard`, `derivation`,
> `mv`, `join`, `export` are all enforced — guard / derivation `ctx.vault` reads
> resolve under their own layer policy (`guard` defaults to the lenient
> `'substitute'`). The **`mv` layer** fires for materialized views that declare
> `{ i18nLocale, i18nFields }` — **UNION** (group-key i18n fields resolve before
> the unified-row bucketing) and **query-form** (resolved in
...
> locale-less). The **`export` layer** fires for
> `exportStream`/`exportJSON({ resolveLabels })` — records collapse to the export
> locale (i18nText to the string, dict `<field>Label`s resolved), with the
> dictionary snapshot omitted. Design:
> `docs/superpowers/specs/2026-06-13-i18n-mv-join-layer-design.md`.
