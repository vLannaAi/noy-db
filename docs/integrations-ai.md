...
Every record can be schema-validated via [Standard Schema v1](https://standardschema.dev) (Zod, Valibot, ArkType, Effect Schema), every mutation is recorded in a tamper-evident hash-chained ledger, history is delta-encoded via RFC 6902 JSON Patch, soft foreign-key references are enforceable per-collection, and backups verify end-to-end on load.
...
## Integrity Feature Reference

Five composable integrity features. Each one is opt-in; a collection with none of them still gets the full encryption + keyring + query DSL stack.
...
Cross-compartment refs are rejected at construction with `RefScopeError`. Dotted-path field names are not supported — top-level fields only.
...
| admin | all | all | admin/operator/viewer/client | same, plus cascade | yes |
| operator | granted | granted | — | — | granted |
| viewer | all | — | — | — | yes |
...
// Export decrypted JSON (ACL-scoped, silently skips
