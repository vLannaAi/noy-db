...
Today the multi-user surface is reachable directly via the always-on core API:
...
The `withTeam()` strategy will gate this surface in a planned follow-up extraction; today setting up keyring/grant code is "free" if you don't import the symbols.
...
- `vault.elevate(tier, { ttlMs, reason })` — scoped tier elevation
...
## Behavior when NOT opted in (after the planned extraction)
...
- Tiers layer on top of roles for hierarchical visibility
