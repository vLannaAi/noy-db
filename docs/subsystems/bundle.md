...
- `vault.buildBundleRecipientKeyrings(recipients)` — produce per-recipient `KeyringFile` records without touching the source adapter (used by the re-keying path; )
