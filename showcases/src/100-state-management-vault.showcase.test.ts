 ...
 *
 * Access the control plane with `db.openStateManagementVault()`.
 *
 * Why it matters
 * ──────────────
...
describe('Showcase 100 — StateManagement Vault', () => {
  it('auto-opens the control plane and records registry + manifest + event', async () => {
    ...
    db.withVaultTemplate('client', { version: 1, configure: (v) => { v.collection('invoices', { indexes: ['buyerId'] }) } })
    ...
    const state = await db.openStateManagementVault()
