...
describe('parseArgs', () => {
  ...
  it('parses --template nuxt-default', () => {
    expect(parseArgs(['--template', 'nuxt-default']).options.template).toBe('nuxt-default')
  })

  it('parses --template vite-vue', () => {
    expect(parseArgs(['--template', 'vite-vue']).options.template).toBe('vite-vue')
  })

  it('parses --template vanilla', () => {
    expect(parseArgs(['--template', 'vanilla']).options.template).toBe('vanilla')
  })

  it('parses --template electron', () => {
    expect(parseArgs(['--template', 'electron']).options.template).toBe('electron')
  })

  it('rejects an unknown template', () => {
    expect(() => parseArgs(['--template', 'svelte'])).toThrow(/electron|vanilla|vite-vue|nuxt-default/)
  })

  it('rejects a missing --template value', () => {
    expect(() => parseArgs(['--template'])).toThrow(/electron|vanilla|vite-vue|nuxt-default/)
  })

  it('parses --sync none', () => {
    expect(parseArgs(['--sync', 'none']).options.syncAdapter).toBe('none')
  })

  it('parses --sync memory', () => {
    expect(parseArgs(['--sync', 'memory']).options.syncAdapter).toBe('memory')
  })

  it('parses --sync file', () => {
    expect(parseArgs(['--sync', 'file']).options.syncAdapter).toBe('file')
  })

  it('parses --sync browser', () => {
    expect(parseArgs(['--sync', 'browser']).options.syncAdapter).toBe('browser')
  })

  it('rejects an unknown --sync value', () => {
    expect(() => parseArgs(['--sync', 's3'])).toThrow(/browser, file, memory, none/)
  })
