/**
 * Hierarchical access — tier-aware keyring helpers (v0.18 #205).
 *
 * The keyring's existing `deks: Map<string, CryptoKey>` is keyed by
 * collection name. v0.18 extends the key space:
 *
 *   `'invoices'`      — tier-0 DEK (unchanged from v0.x)
 *   `'invoices#1'`    — tier-1 DEK
 *   `'invoices#2'`    — tier-2 DEK
 *
 * Tier 0 keeps the bare collection name so every pre-v0.18 keyring
 * keeps working without migration. Tiers ≥ 1 use `#N` suffixes that
