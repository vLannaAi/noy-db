    ...
    if (!coll) {
      ...
      const collOpts: ConstructorParameters<typeof Collection<T>>[0] = {
        ...
        ...(this.derivationRegistry !== null
          ? (this.derivationRegistry.validateFieldsFor(collectionName, options?.schema, Object.keys({ ...options?.fieldMeta, ...options?.moneyFields, ...options?.dictKeyFields, ...options?.refs, ...options?.computed })), {
              derivationSource: {
