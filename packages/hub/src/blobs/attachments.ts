 ...
 * updated. A `vault.blobGC()` call removes orphaned chunks after
...
export class AttachmentHandle {
   ...
   * Unreferenced blobs are cleaned up lazily by `vault.blobGC()`,
    ...
    const body = new ReadableStream<Uint8Array>({
      async start(controller) {
        try {
          const chunks: Uint8Array[] = []
          for (let i = 0; i < chunkCount; i++) {
            const chunk = await readChunk(eTag, i, blobDEK)
            if (!chunk) {
              controller.error(
