/**
 * Golden export-surface freeze for the `@noy-db/hub/to` seam (S5 family doors).
 *
 * The `/to` subpath is the ciphertext-facing contract every `to-*` store in
 * noy-db-to binds to — the family door that supersedes the deprecated
 * `/adapter` alias. This test freezes its export list against a checked-in
 * baseline (`to-surface.golden.json`) so drift fails CI — adding requires a
 * visible baseline update, removing / renaming fails loudly.
 *
 * MECHANISM — identical to the `/adapter` golden test (see its header for the
...
const baseline: Surface = JSON.parse(read('./to-surface.golden.json')) as Surface
const parsed = parseExports(read('../src/kernel/to/index.ts'))

describe('@noy-db/hub/to — golden export surface', () => {
  it('value exports match the frozen baseline (runtime enumeration)', () => {
    const runtime = Object.keys(to)
      .filter((k) => (to as Record<string, unknown>)[k] !== undefined)
      .sort()
