...
describe('Query.join() — v0.6 eager single-FK joins', () => {
  let db: Noydb
  ...
  // ─── design-forward partition seams ─────────────────────────

  it('every JoinLeg in the plan carries partitionScope: "all" (v0.6 constraint #1)', async () => {
    await seedCompartment(db)
    const c = await db.openVault('TEST')
    const invoices = c.collection<Invoice>('invoices', {
      refs: { clientId: ref('clients') },
