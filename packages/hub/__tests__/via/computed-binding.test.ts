...
describe('computedVia (#638 Task 7)', () => {
  it('declares the computed brand + the fixed virtual posture (queryable: none, unconditionally)', () => {
    const b = computedVia({ virtualFields: new Map([['doubled', virtualDescriptor((r) => (r.amount as number) * 2)]]) })
  ...
  it('covers only its declared virtual fields', () => {
    const b = computedVia({ virtualFields: new Map([['doubled', virtualDescriptor((r) => r.amount)]]) })
