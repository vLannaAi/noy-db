...
async function seed() {
  const db = await createNoydb({ store: toMemory(), user: 'a', secret: 'pw-hybrid', searchStrategy: withSearch() })
...
describe("retrieve({ mode: 'hybrid' })", () => {
  ...
  it('throws when the collection has no embeddings', async () => {
    const db = await createNoydb({ store: toMemory(), user: 'a', secret: 'pw-hybrid-no-emb', searchStrategy: withSearch() })
    const v = await db.openVault('v')
