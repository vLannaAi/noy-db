/**
 * Golden export-surface freeze for the `@noy-db/hub/on` seam (S5 family doors).
 *
 * An unlock / auth primitive (`on-password`, `on-webauthn`, `on-oidc`,
 * `on-totp`, `on-shamir`, `on-recovery`, …) binds ONLY to this subpath: the
 * `mintPaperRecoveryEntry` paper-recovery mint primitive (value) plus the
 * keyring/role/permission/rewrap/enroll shapes those packages read and
 * write (types). This test freezes its export list against a checked-in
 * baseline (`on-surface.golden.json`) so drift fails CI — adding requires a
...
import * as on from '../src/kernel/on/index.js'
import type {
  UnlockedKeyring, Role, Permissions, SlotRewrapContext, EnrollAuthenticatorOptions, PaperRecoveryEntry,
} from '../src/kernel/on/index.js'
...
const parsed = parseExports(read('../src/kernel/on/index.ts'))

describe('@noy-db/hub/on — golden export surface', () => {
  it('value exports match the frozen baseline (runtime enumeration)', () => {
    const runtime = Object.keys(on)
      .filter((k) => (on as Record<string, unknown>)[k] !== undefined)
      .sort()
