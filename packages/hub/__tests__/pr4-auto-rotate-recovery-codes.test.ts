 ...
 * `db.recoverSecret` defaults to replacing ALL remaining recovery
...
describe('db.recoverSecret auto-rotate (#36)', () => {
  it('default: rotates 7 remaining codes after consuming 1 of 8', async () => {
    ...
    const result = await db.team.recoverSecret('acme', {
      newSecret: PHRASE_AFTER_RECOVERY_1,
  ...
  it('original remaining codes are invalid after rotation', async () => {
    ...
    await expect(
      reopen.team.recoverSecret('acme', {
        newSecret: PHRASE_AFTER_RECOVERY_2,
