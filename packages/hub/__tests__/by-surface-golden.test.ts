/**
 * Golden export-surface freeze for the `@noy-db/hub/by` seam (S5 family doors).
 *
 * A session-share transport (`by-tabs`, `by-peer`) binds ONLY to this
 * subpath: the drain-barrier coordination port for the schema-fence
 * cutover — `isQuorum` / `runDrainBarrier` (values) plus `CoordinationProvider`
 * / `WriterPresence` / `FenceState` / `DrainBarrierOptions` (types). This test
 * freezes its export list against a checked-in baseline (`by-surface.golden.json`)
...
import * as by from '../src/kernel/by/index.js'
import type {
...
const parsed = parseExports(read('../src/kernel/by/index.ts'))

describe('@noy-db/hub/by — golden export surface', () => {
  it('value exports match the frozen baseline (runtime enumeration)', () => {
    const runtime = Object.keys(by)
      .filter((k) => (by as Record<string, unknown>)[k] !== undefined)
      .sort()
...
type _FrozenTypes = [CoordinationProvider, WriterPresence, FenceState, DrainBarrierOptions]
