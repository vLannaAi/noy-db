/**
 * Tests for ScanBuilder + Collection.scan().aggregate() — v0.6.
 *
 * Covers:
 ...
 *   - Same reducer protocol as Query.aggregate()
 *   - O(reducers) memory guarantee via a 5k-record streaming test
 *. seed seam still honored in the scan path
...
describe('ScanBuilder > .aggregate() terminal', () => {
  ...
  it('constraint #2 — seed parameter plumbed through (no-op in v0.6)', async () => {
