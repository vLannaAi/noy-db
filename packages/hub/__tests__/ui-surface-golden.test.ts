/**
 * Golden export-surface freeze for the `@noy-db/hub/ui` seam (S5 family doors).
 *
 * `/ui` is the narrow, stable `collection.describe()` output contract that
 * `@noy-db/ui` (and `@noy-db/ui-nuxt`) bind to instead of the whole
 * `@noy-db/hub` root barrel. It is the relocated `src/describe.ts` (moved to
 * `src/kernel/ui/index.ts`); the old `/describe` subpath is now a deprecated
 * alias built from the same source (see the header of `kernel/ui/index.ts`).
 * This test freezes its export list against a checked-in baseline
 * (`ui-surface.golden.json`) so drift fails CI — adding requires a visible
 * baseline update, removing / renaming fails loudly.
 *
 * MECHANISM — identical to the `/to` golden test (see its header for the
 * rationale). `/ui` is type-only (`describe()` itself stays a `Collection`
 * method reached via the root barrel / `/kernel`), so the runtime VALUE
...
describe('@noy-db/hub/ui — golden export surface', () => {
  it('value exports match the frozen baseline (runtime enumeration)', () => {
    const runtime = Object.keys(ui)
      .filter((k) => (ui as Record<string, unknown>)[k] !== undefined)
      .sort()
